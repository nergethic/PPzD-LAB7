...
public class PlayerController : MonoBehaviour {
    ...
    void Initialise() {
        ...
        banditController.GotHitEvent     += () => HandleGotHitEvent(banditController.HealthNormalized);
    ...
    void HandleGotHitEvent(float newHealthPercentage) {
        var playerAudioControllerSubsystem = subsystems.OfType<PlayerAudioController>().First();
        if (playerAudioControllerSubsystem != null)
            playerAudioControllerSubsystem.UpdatePlayerAudioData(newHealthPercentage);
        
        foreach (var playerSubsystem in subsystems)
            playerSubsystem.HandleEvent(PlayerEventType.GotHit);
    }
